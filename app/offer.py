...
def size_change_px(data):
    return (data*72)/96
def offer(date,months,amount,supervisor,see,rolePost):
    # date = date
    # months = months
    # amount = amount
    buffer = io.BytesIO()
    ...
    p.drawString(size_change_px(60), top-size_change_px(36)-(px7*4), str(date))
    p.drawString(size_change_px(60), top-size_change_px(36)-(px7*7), str(see.f_name) +" "+ str(see.l_name))
    p.drawString(size_change_px(60), top-size_change_px(36)-(px7*10), str(see.edu_collage_name))
    p.drawString(size_change_px(60), top-size_change_px(36)-gap-(px7*13), f"Sub: Internship for {months} months")
    p.drawString(size_change_px(60), top-size_change_px(36)-(gap*2)-(px7*16), f"Dear {see.f_name}")
    mystyle = ParagraphStyle('style',
    fontSize = px7*2,
    leading= (px7*2)+px5)
    p1 = Paragraph(f'''We are pleased to offer you an educational internship opportunity as {rolePost}.
    You will report directly to {supervisor} Your immediate supervisor will be {supervisor}.
    ...
    This offer letter must be signed within 10 days from {date}. Please send asigned
